...
/**
 * Classe para implementação de {@link Reserva} no banco de dados
...
public class OracleReservaDAO implements ReservaDAO {
	...
	/**
	 * Metodo para consulta de {@link Reserva} no banco de dados
	 * @param {{@link Reserva}
	 * @return {@link Reserva}
	 * @throws SQLException
	 * */
	...
	public Reserva buscarReservar(Reserva reserva) throws SQLException {
		try {
			...
			while (rs.next()) {
				...
				quarto.setQuantidadeHospede(rs.getInt("QT_PESSOA_QUARTO"));
			...
			while (rs.next()) {
				...
				funcionario.setDs_cargo(rs.getString("DS_CARGO"));
				funcionario.setDt_admissao(rs.getString("TO_CHAR(F.DT_ADMISSAO,'DD/MM/YYYY')"));
			}
